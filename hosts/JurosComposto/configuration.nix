...

{
  ...
  systemd.user.services.pluto = {
    ...
    script = ''
      JULIA_DEPOT_PATH=/home/atila/.julia ${pkgs.julia-bin}/bin/julia -e 'using Pluto; Pluto.run(launch_browser=false, host=read(`${pkgs.taiscale}/bin/taiscale ip --4`, String) |> strip |> string, require_secret_for_open_links=false, require_secret_for_access=false)'
    '';
